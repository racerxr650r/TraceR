...
| 5b    | Inline AI assistance — TypeScript surfaces. | ⏳ Not started — `@projectspec` chat participant via `vscode.chat.createChatParticipant` with slash commands `/draft-hlr`, `/draft-llr`, `/draft-test`, `/draft-module`, `/draft-pvd`, `/expand`, `/review`, `/suggest-traces`, `/gap-fill`; schema-driven AI tree context-menu entries from `ui_hints_index.aiActions`; AI-suggest Quick Fix variants on the Phase 2.5c table; diff-preview-and-apply flow with backup; settings UI for `projectXml.ai.*`; graceful degradation when no LM / untrusted workspace. |
