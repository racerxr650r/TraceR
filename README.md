...
| `projectXml.warningsAsErrors`| `false`             | Escalate lint warnings to error severity in the Problems panel and the status bar (HLR-042). NEVER suppresses warnings — only raises their severity. |
...
tools/
  ...
  vscode-project-xml/  # VS Code extension (Phases 1 + 2 + 2.5 + 2.5b + 2.5c + 3 + 4 + 5a + 5b + 5.5 + 6)
                       # `dist/python/` is generated by `npm run prepackage`
                       # and ships inside the .vsix (HLR-060).
